...
// New creates a new sx127x connection. The SPI wire must already be configured.
//
...
func New(bus drivers.SPI, resetPin, csPin machine.Pin, dio0Pin machine.Pin) *Device {
	return &Device{
		bus:      bus,
		resetPin: resetPin,
		csPin:    csPin,
		dio0Pin:  dio0Pin,
		iChan:    make(chan bool, 1),
	}
}
...
func (d *Device) Configure(cfg Config) error {
	...
	if d.dio0Pin != machine.NoPin {
		d.dio0Pin.Configure(machine.PinConfig{Mode: machine.PinInputPulldown})
		d.dio0Pin.SetInterrupt(machine.PinRising, d.handleInterrupt)
	}
...
// RxTo tries to receive a LoRa packet (with timeout in milliseconds)
//
...
//
// CRC failures are considered as errors, the caller can simply re-listen
func (d *Device) RxTo(timeoutMs uint32, buf []uint8) (int, error) {
